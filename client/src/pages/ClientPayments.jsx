...
export default function ClientPayments() {
  ...
  const pickClient = (c) => {
    setSelClient(c); setQ(c.name); setIdx(-1); setSelectedInvIds(new Set());
    api.get(`/client-invoices?client_id=${c.id}`).then((rows) => setInvs((rows || []).filter((r) => r.balance > 0)));
  };

  const resetSearch = () => { setSelClient(null); setQ(''); setInvs([]); setSelectedInvIds(new Set()); };
  ...
  const recordPayment = () => {
    if (selectedInvIds.size === 0) {
      nav(`/client-payments/new?client=${selClient.id}`);
    } else {
      const invoiceIds = Array.from(selectedInvIds).join(',');
      nav(`/client-payments/new?client=${selClient.id}&invoices=${invoiceIds}`);
    }
  };
